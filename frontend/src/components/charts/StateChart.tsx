    ...
    <div className="rounded-lg border border-[#E4E8F4] bg-white px-3 py-2 shadow-lg text-xs">
      ...
      {payload.map((p: any) => (
        <p key={p.dataKey} style={{ color: p.color }} className="font-medium">
          {p.name}: <span className="font-bold">{p.value.toLocaleString("en-IN")}</span>
        </p>
      ))}
    </div>
  );
...
export function StateChart({ data, title = "Accidents by district (top 10)" }: Props) {
  ...
  return (
    <div style={{ width: "100%", height }}>
      <ResponsiveContainer width="100%" height="100%">
        ...
        >
          <CartesianGrid strokeDasharray="3 3" horizontal={false} stroke="#EDF0F8" />
          <XAxis
            type="number"
            axisLine={false}
            tickLine={false}
